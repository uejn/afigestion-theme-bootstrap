/**
 * Mesas AJAX CRUD
 * Maneja crear, editar y eliminar mesas sin refrescar la pantalla.
...
(function ($) {
    ...
    var MesasCrud = {
        ...
        /**
         * Intercepta el submit del formulario dentro del modal de editar_mesa
         */
        bindFormSubmit: function () {
            var self = this;
            $(document).on('submit', '#ajaxModal .ajax-mesa-form, #ajaxModal form[id*="MesaEditar"]', function (e) {
                e.preventDefault();
                var $form = $(this);
                ...
                $.ajax({
                    url: self.baseUrl + 'ajax_guardar_mesa',
                    type: 'POST',
                    data: formData,
                    dataType: 'json',
        ...
        bindDeleteButtons: function () {
            ...
            $(document).on('click', '.ajax-delete-mesa', function (e) {
                e.preventDefault();
                var $btn = $(this);
                var mesaId = $btn.data('id');
                var mesaName = $btn.data('name');

                if (!confirm('¿Está seguro de eliminar la Mesa ' + mesaName + '?')) {
                    return;
                }
                ...
                $.ajax({
                    url: self.baseUrl + 'ajax_eliminar_mesa/' + mesaId,
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        _method: 'POST'
        ...
        buildEdificioHtml: function (row) {
            ...
            var text = row.edificio_calle ? (row.edificio_calle + ' ' + row.edificio_numero) : row.edificio_name;
            return '<a href="/afigestion/edificios/editar_edificio/' + row.edificio_id + '" ' +
                'class="ajax-modal">' + this.escapeHtml(text) + '</a>';
        },
        ...
        buildActionsHtml: function (row) {
            var editBtn = '<a href="' + this.baseUrl + 'editar_mesa/' + row.id + '" ' +
                'class="btn btn-info btn-xs ajax-modal" escape="false">' +
                '<span class="glyphicon glyphicon-pencil"></span></a>';

            var deleteBtn = '<a href="#" class="btn btn-danger btn-xs ajax-delete-mesa" ' +
        ...
        buildRowHtml: function (row) {
            ...
            var html = '<tr id="mesa-row-' + row.id + '">';
        ...
        addRowToTable: function (row) {
            var $tbody = $('.table-listado-mesas tbody');
            if ($tbody.length === 0) {
        ...
        /**
         * Actualiza el contador de mesas
         */
            ...
            if (errors) {
                for (var field in errors) {
                    if (errors.hasOwnProperty(field)) {
                        var $input = $form.find('[name="data[Mesa][' + field + ']"]');
                        if ($input.length) {
